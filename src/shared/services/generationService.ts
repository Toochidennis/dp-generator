...
export const generationService = {
  getGenerations(signal?: AbortSignal): Promise<Generation[]> {
    ...
    return apiRequest<Generation[]>("/api/generations.php", { signal });
  },
  ...
  getGenerationsByProgram(programId: string, signal?: AbortSignal): Promise<Generation[]> {
    ...
    return apiRequest<Generation[]>(`/api/generations.php?programId=${encodeURIComponent(programId)}`, { signal });
  },
  ...
  getPublicGeneration(generationId: string, signal?: AbortSignal): Promise<PublicGeneration> {
    ...
    return apiRequest<PublicGeneration>(`/api/generations.php?id=${encodeURIComponent(generationId)}`, { signal });
  },
};
